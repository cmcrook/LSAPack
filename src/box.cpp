...
{
	PROFILE_SCOPE("CollisionChecker");
	int i = c.i;
	int j = c.j;
...
{
	PROFILE_SCOPE("FindNextTransfer");
	double ttime = dblINF;
...
{
	PROFILE_SCOPE("PredictCollision");
	double ctimej;
...
Event Box::findNextCollision(int i) {
	PROFILE_SCOPE("FindNextCollision");
	Collision cc(i, this);
...
void Box::writeConfiguration(const char* wconfigfile, int iteration) {
	...
	//output << "\teventspercycle : " << eventspercycle << std::endl;
	//output << "\tN : " << N << std::endl;
	//output << "\tinitialpf : " << initialpf << std::endl;
	//output << "\tmaxpf : " << maxpf << std::endl;
	//output << "packing fraction = " << pf << std::endl;
	//output << "\ttemp : " << temp << std::endl;
	//output << "\tgrowthrate : " << growthrate << std::endl;
	//output << "\tmaxSizeChange: " << maxSizeChange << std::endl;
	//output << "\tciter: " << citer << std::endl;
	//output << "\thardwallBC : " << hardwallBC << std::endl;
...
void Box::writeLAMMPSDump(const char* wconfigfile, int iteration) {
	int count;
...
//Formatting
	...
	output << "0.0 " << SIZE << std::endl;
	output << "0.0 " << SIZE << std::endl;
	output << "0.0 " << SIZE << std::endl;

	output << "ITEM: ATOMS id type x y z radius" << std::endl;

	//Write position and radius
	for (int i = 0; i < N; i++) {
		output << s[i].id << " " << s[i].species << " ";
