...
void box::AssignCells()
{
    //for (int i = 0; i < N; i++)
    //{
    //    std::cout << i << ": " << s[i].x[0] << ", " << s[i].x[1] << ", " << s[i].x[2] << std::endl;
    //}
...
int box::Optimalngrids()
{
    ...
    double largest_particle = -1.0;
    for (const auto& s : particle_sizes) {
        if (largest_particle < 0 || s > largest_particle) {
            largest_particle = s;
        }
    }
...
void box::WriteConfiguration(const char* wconfigfile, int iteration)
{
  ...
  std::ofstream output;
  if(iteration == 0)
      output = std::ofstream (wconfigfile);
  else
      output = std::ofstream (wconfigfile, std::ios_base::app);
